...
ResultSet SqlExecutor::execSimpleQuery(
    ...
    Assert::nonNull(target, "at SqlExecutor::execSimpleQuery: can not find " +
                            tableName + " table");

    // 解析表达式
    Optional<BiRelation> bi = SqlParser::toBiRelation(logicalExpression);

    Assert::isFalse(bi.empty(),
                    "at SqlExecutor::execSimpleQuery: expression error! " +
                    logicalExpression);
    ...
    Assert::nonNull(
            col, "at SqlExecutor::execSimpleQuery: can not find column called " +
                 leftOperand);
...
bool SqlExecutor::eval(ColumnPtr column,
...
SqlExecutor::SqlExecutor() : tables(nullptr) {}
